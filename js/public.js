...
$(function() {
    ...
    $(".change_page").length <= 0 && $("body").append($("<div/>").addClass("change_page").append([$("<div/>").addClass("icon-chevron-thin-up"), $("<div/>").addClass("icon-chevron-thin-down")]));
    $("#logo").length <= 0 && $("body").append($("<div/>").attr("id", "logo").text("by Chestnut Wu"));

    $(".icon-chevron-thin-up, .icon-chevron-thin-down").click(function() {
      if (null === $a)
        return false;
      if (-1 === (index = $a.index($a.filter(".active"))))
        return false;
